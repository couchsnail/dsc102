...
def process(csv_name):
    ...
    #ddf['reviewTime'].map(lambda x: eval(x[-4:]) if isinstance(x, str) else x, meta=('reviewTime', 'object'))
    #.to_datetime(ddf['reviewTime'], errors='coerce').dt.year
    #.map(lambda x: (x % 31536000) + 1970)
    # Parse helpful field
    ddf['helpful_votes'] = ddf['helpful'].map(lambda x: eval(x)[0] if isinstance(x, str) else x[0], meta=('helpful_votes', int))
    ddf['total_votes'] = ddf['helpful'].map(lambda x: eval(x)[1] if isinstance(x, str) else x[1], meta=('total_votes', int))
    ...
    result = ddf.map_partitions(lambda df: df.groupby('reviewerID').agg(agg).rename(columns = rename).reset_index())
